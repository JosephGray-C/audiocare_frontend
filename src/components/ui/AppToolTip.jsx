...
export default function AppToolTip({ message, children, position = "right", className = "" }) {
    const [hovered, setHovered] = useState(false);
    const [focused, setFocused] = useState(false);
    const [positioned, setPositioned] = useState(false);
    const [coords, setCoords] = useState({ top: -9999, left: -9999 });
    const hasMessage = Boolean(message);
    const resolvedPosition = ["top", "bottom", "left", "right"].includes(position) ? position : "right";
    ...
    return (
        <div
            ...
            onMouseEnter={() => hasMessage && setHovered(true)}
            onMouseLeave={() => hasMessage && setHovered(false)}
