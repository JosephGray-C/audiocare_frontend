...
export default function SearchableSelect({
    ...
    const filtered = options.filter(
        o =>
            o.label.toLowerCase().includes(search.toLowerCase()) || (o.sublabel && o.sublabel.toLowerCase().includes(search.toLowerCase())),
    );
    ...
    return (
        <div ref={containerRef} className='relative'>
            ...
            <button
                ...
                `}
            >
                {Icon && <Icon size={15} className={`mr-2.5 shrink-0 ${error ? "text-red-400" : "text-slate-400"}`} />}
                ...
                {selected && !disabled ? (
                    <X size={14} className='ml-2 shrink-0 text-slate-400 hover:text-slate-600 cursor-pointer' onClick={handleClear} />
                ) : (
                    <ChevronDown
                        size={14}
                            ...
                            filtered.map(option => (
                                <button
                                    ...
                                    className={`
                                        ...
                                        ${option.value === value ? "bg-[#34c3d6]/10 text-[#34c3d6]" : "text-slate-700 hover:bg-slate-50"}
                                    `}
                                >
                                    <span className='text-sm font-medium truncate'>{option.label}</span>
                                    {option.sublabel && <span className='text-xs text-slate-400 truncate'>{option.sublabel}</span>}
                                </button>
                            ))
                        )}
                    </div>
