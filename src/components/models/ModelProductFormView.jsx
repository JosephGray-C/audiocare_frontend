...
export default function ModelProductFormView({ model = null, onSaved }) {
  ...
  function validateForm() {
    ...
    if (!formData.costFabricCrc) {
      newErrors.costFabricCrc =
        "El precio de fábrica en colones es obligatorio";
    } else if (Number(formData.costFabricCrc) <= 0) {
      newErrors.costFabricCrc =
        "El precio de fábrica en colones debe ser un número positivo";
    }
  ...
  return (
    <div className="w-full">
      <div className="max-w-5xl mx-auto space-y-5">
        <ModulePanelHeader
          title={
            isEdit ? "Editar Modelo Producto" : "Registrar Modelo Producto"
          }
          subtitle={
            isEdit
              ? "Actualice la información del modelo seleccionado"
              : "Complete los datos para registrar un nuevo modelo"
          }
          icon={Package}
          variant="form"
        />
