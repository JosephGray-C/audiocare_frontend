...
export default function AdminsListView({
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, refreshKey, setCurrentPage]);
  ...
  function fullName(admin) {
    return [admin.name, admin.lastName1, admin.lastName2]
      .filter(Boolean)
      .join(" ");
  }
  ...
  function countActivePermissions(admin) {
    ...
    return active === 0
      ? "Sin permisos"
      : `${active} permiso${active !== 1 ? "s" : ""} activo${active !== 1 ? "s" : ""}`;
  }
        ...
        {!admin.isMaster && (
          <AppToolTip message="Gestionar permisos" position="top">
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-[#ef7d2d]/10 hover:text-[#ef7d2d]"
            >
              <Shield size={14} />
            </button>
          </AppToolTip>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="divide-y divide-slate-100 md:hidden">
          {loadingData ? (
            ...
            <div className="px-5 py-16 text-center">
              <div className="flex flex-col items-center gap-2 text-slate-400">
                ...
                <span className="text-sm">
                  {search ? "Sin resultados" : "No hay admins"}
                </span>
              </div>
            </div>
          ) : (
            paginatedAdmins.map((admin) => {
              ...
              return (
                <article key={admin.id} className="space-y-3 px-5 py-4">
                  <div className="flex items-start gap-3">
                    ...
                    >
                      {admin.isMaster ? (
                        <Crown size={16} className="text-[#ef7d2d]" />
                      ) : (
          ...
          <table className="w-full table-fixed text-[13px]">
            <thead>
              <tr className="border-t border-slate-100">
                ...
                <th className="w-[14rem] px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                  Correo
                </th>
        ...
        {!loadingData && filtered.length > 0 && (
          <div className="flex flex-wrap items-center gap-x-4 gap-y-3 border-t border-slate-100 px-5 py-3">
            <p className="text-xs text-slate-400">
              Mostrando {visibleStart}–{visibleEnd} de {filtered.length} admin
              {filtered.length !== 1 ? "s" : ""}
            </p>
