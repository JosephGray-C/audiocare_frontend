...
export default function ProductFormView({ product = null, onSaved }) {
    ...
    useEffect(() => {
        async function loadOptions() {
            ...
            try {
                const [models, orders] = await Promise.all([getModelProducts(), getSupplierOrders()]);
            ...
            <div className='max-w-4xl mx-auto space-y-5'>
                ...
                <form id='productForm' onSubmit={handleSubmit} className='space-y-5'>
                    <div className='bg-white rounded-2xl border border-slate-200 shadow-sm p-5 lg:p-6 space-y-4'>
                        <h2 className='text-sm font-semibold text-slate-700 uppercase tracking-wider'>Datos del producto</h2>
                        ...
                        <div className='rounded-xl bg-slate-50 border border-slate-200 px-4 py-3'>
                            <p className='text-xs text-slate-500 leading-relaxed'>
                                El producto se registrará con estado <span className='font-semibold text-emerald-600'>DISPONIBLE</span> y se
                                generará un movimiento de inventario automáticamente.
                            </p>
                        </div>
                    </div>
                </form>
