...
export default function SupplierOrderFormView({ order = null, onSaved }) {
  ...
  const formattedTotalCrc = useMemo(
    () => formatCRC(formData.totalAmountCrc),
    [formData.totalAmountCrc],
  );
  ...
  const formattedInsuranceCrc = useMemo(
    () => formatCRC(formData.insuranceCrc),
    [formData.insuranceCrc],
  );
  ...
  async function handleSubmit(e) {
    ...
    const payload = {
      ...
      insuranceEur: formData.insuranceCrc
        ? Number(convertCRCToEuro(formData.insuranceCrc))
        : 0,
    };
    ...
    <div className="w-full">
      <div className="max-w-4xl mx-auto space-y-5">
        <ModulePanelHeader
          title={
            isEdit
              ? "Editar Pedido del Proveedor"
              : "Registrar Pedido del Proveedor"
          }
          subtitle={
            isEdit
              ? "Actualice la información del pedido seleccionado"
