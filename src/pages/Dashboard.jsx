...
export default function Dashboard() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      setLoading(true);
      const [products, orders, models, supplierOrders, clients, movements] =
        await Promise.all([
          getAllProducts(),
          getOrders(),
          getModelProducts(),
  ...
  const metrics = useMemo(() => {
    ...
    const totalCost = billed.reduce(
      (s, p) => s + Number(p.model?.costFabricCrc || 0),
      0,
    );
    ...
    const inventoryValue = available.reduce(
      (s, p) => s + Number(p.model?.priceSale || 0),
      0,
    );
    const inventoryCost = available.reduce(
      (s, p) => s + Number(p.model?.costFabricCrc || 0),
      0,
    );
    ...
    billed.forEach((p) => {
      ...
      if (mid) {
        if (!modelSalesMap[mid])
          modelSalesMap[mid] = { name, count: 0, revenue: 0 };
        modelSalesMap[mid].count++;
        modelSalesMap[mid].revenue += Number(p.model?.priceSale || 0);
      }
    });
    const topModels = Object.values(modelSalesMap)
    ...
    // Supplier investment
    const totalSupplierInvestment = supplierOrders.reduce(
      (s, o) => s + Number(o.totalAmountCrc || 0),
      0,
    );
    const totalSupplierInvestmentEur = supplierOrders.reduce(
      (s, o) => s + Number(o.totalAmountEur || 0),
      0,
    );
    ...
    return {
      ...
      clients,
      supplierOrders,
      totalSupplierInvestment,
      totalSupplierInvestmentEur,
    };
  }, [data]);
  ...
  const maxProducts = Math.max(
    ...metrics.monthlySales.map((m) => m.products),
    1,
  );
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          ...
          <div className="space-y-3">
            ...
            <FinancialRow
              ...
              value={fmtCRC(metrics.totalCost)}
            ...
            <div className="border-t border-slate-100 pt-3">
              <FinancialRow
                ...
                color={
                  metrics.grossProfit >= 0 ? "text-emerald-600" : "text-red-500"
                }
                bold
              />
            </div>
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-sm text-slate-500">
                Margen:{" "}
                <span
                  className={`font-semibold ${metrics.margin >= 0 ? "text-emerald-600" : "text-red-500"}`}
                >
            ...
            <div className="border-t border-slate-100 pt-3 mt-3 flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">
                Inversión en proveedor:{" "}
                {fmtCRC(metrics.totalSupplierInvestment)} (
                {metrics.supplierOrders.length} pedido
                {metrics.supplierOrders.length !== 1 ? "s" : ""})
              </span>
            </div>
          </div>
        </div>
              ...
              >
                ...
                >
                  <div
                    ...
                    style={{
                      height: `${Math.max((m.products / maxProducts) * 100, m.products > 0 ? 8 : 2)}%`,
                    }}
                  />
                </div>
                <span className="text-[10px] text-slate-400">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          ...
          {metrics.lowStockModels.length === 0 &&
              ...
              {metrics.modelsNoStock > 0 && (
                <div className="px-3 py-2.5 rounded-lg bg-red-50 border border-red-100">
                  <p className="text-xs font-medium text-red-600">
                    {metrics.modelsNoStock} modelo
                    {metrics.modelsNoStock !== 1 ? "s" : ""} sin stock
                  </p>
                </div>
              )}
              {/* Low stock models */}
                ...
                <div key={mov.id} className="flex items-start gap-2.5 py-1.5">
                  <div
                    className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${
                      mov.eventType === "PRODUCT_ADDED"
                        ? "bg-emerald-400"
                        : mov.eventType === "PRODUCT_SOLD"
                          ? "bg-blue-400"
                          : mov.eventType === "PRODUCT_SALE_CANCELED"
                            ? "bg-amber-400"
                            : "bg-purple-400"
                    }`}
                  />
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <QuickAction
            label="Ver Pedidos"
            icon={Truck}
            onClick={() => navigate("/pedidos-proveedor")}
            color="#64748b"
          />
        </div>
      </div>
...
function FinancialRow({ label, value, color, bold }) {
  return (
    <div className="flex justify-between items-center">
      <span
        className={`text-sm ${bold ? "font-semibold text-slate-700" : "text-slate-500"}`}
      >
        {label}
      </span>
